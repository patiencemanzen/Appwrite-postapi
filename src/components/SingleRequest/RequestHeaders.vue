    ...
    <div>
      ...
      <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                  <tr v-for="header in customHeaders" :key="header.key+Math.random().toString(16).slice(2)" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
...
export default {
  ...
  watch: {
    headers: {
      handler(propHeaders) {
        if (propHeaders) {
          this.customHeaders = propHeaders;
        }
      },
      immediate: true,
    },
    customHeaders: {
      handler(newCustomHeaders) {
        this.$root.$emit("new_headers", newCustomHeaders);
      },
      immediate: true,
    },
  },
