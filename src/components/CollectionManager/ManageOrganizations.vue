...
<template>
    <div v-if="isOpen" class="translate-x-0 fixed top-5 right-5 bottom-5 rounded-2xl z-40 p-4 overflow-y-auto transition-transform bg-white w-[30vw] shadow-lg border border-gray-200 dark:bg-gray-800">
        ...
        <div class="border border-gray-200 mt-4 rounded-[10px] bg-clip-border bg-[#f5f7fe] shadow-3xl shadow-shadow-500 p-2 overflow-y-auto h-auto">
          <ul v-if="!isEmpty(organizations)" class="p-0 m-0">
            <li class="cursor-pointer mb-2" v-for="organization in organizations" :key="organization.$id">
              <a class="hover:bg-gray-100 transition border border-gray-200 flex items-center p-2 text-gray-900 rounded-lg dark:text-white bg-gray-50 dark:hover:bg-gray-700">
